...
This document provides a template for running through HydroBOT in a single document, retaining everything in-memory (no intermediate saving). Intermediate saving is a very simple flip of a switch, demoed in [its own doc](workflow_save_steps.qmd).
...
This is not actually run here for speed- the same thing is done in a notebook for the [workflow saving each step](../workflows/workflow_save_steps.qmd).
