...
To run HydroBOT, we need to provide paths to directories for input data and output data, as well as arguments for the aggregation.
...
Use the `scenario_example/` directory [created](../provided_data/scenario_creation.qmd) to capture a very simple demonstration case of 46 gauges in three catchments for 10 years.
...
To determine what to save and what to return to the active session, use `outputType` and `returnType`, respectively. Each of them can take a list of any of `'none'`, `'summary'`, `'yearly'`, `'all_events'` `'all_successful_events'`, and `'all_successful_interEvents'`. For this demonstration I'll just use `summary` and `'yearly'` because that is needed for EWR assessment and not return anything to memory (we do that in [a different notebook](workflow_in_memory.qmd).
...
Here, I'm using an interleaved list of theme and spatial aggregations (see [the detailed docs](../aggregator/theme_space_agg.qmd) for more explanation) following an initial aggregation over the full time, and applying only a single aggregation function at each step for simplicity. Those steps are specified a range of different ways to give a small taste of the flexibility here, but see the [spatial](../aggregator/spatial_agg.qmd) and [theme](../aggregator/theme_agg.qmd) docs for more examples.
...
```{r}
aggseq_ewr <- list(all_time = 'all_time',
               ewr_code = c('ewr_code_timing', 'ewr_code'),
               sdl_units = sdl_clip,
               Target = c('ewr_code', 'Target'))


funseq_ewr <- list('ArithmeticMean',
...
In general, we will likely want to go from EWRs to their planning units, then assess environmental objectives, and then get to SDL units or Specific goals (or vice-versa), and then Targets or longer-term Objectives). A reasonable standard aggregation is:

```{r}
target_aggseq <- list(all_time = 'all_time',
                      ewr_code = c('ewr_code_timing', 'ewr_code'),
                      planning_unit = planning_units,
                      env_obj =  c('ewr_code', "env_obj"),
                      sdl_units = sdl_clip,
...
target_funseq <- list('ArithmeticMean',
                      'CompensatingFactor',
                      'ArithmeticMean',
                      'ArithmeticMean',
                      'SpatialWeightedMean',
...
To check here that we did in fact run HydroBOT, we read in the data and make a quick (crude) plot.
