...
This document provides a template for running through the toolkit, saving the output of each step along the way in a single document. Retaining everything in-memory is a very simple flip of a switch, demoed in [its own doc](WERP_toolkit_in_memory). See the [repo readme](WERP_toolkit_demo.qmd) for overall structure of the repo and package installation.
