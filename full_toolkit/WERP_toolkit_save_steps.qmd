...
# Manual fix of EWR output here:
# using read_EDIT_and_agg instead of read_and_agg

source(file.path(project_dir, "temporary_files/read_EDIT_and_agg.R"))

if (params$REBUILD_DATA) {
  aggout <- read_EDIT_and_agg(datpath = ewr_results,
             type = aggType,
             geopath = bom_basin_gauges,
             causalpath = causal_ewr,
             groupers = agg_groups,
             ...
             saveintermediate = TRUE,
             namehistory = FALSE,
             keepAllPolys = FALSE, #Was TRUE in prior version
             returnList = aggReturn,
             savepath = agg_results)
}
