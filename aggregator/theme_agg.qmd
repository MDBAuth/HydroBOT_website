...
The causal relationships for the EWR tool are provided in the [{HydroBOT}](https://github.com/galenholt/HydroBOT) package as `causal_ewr`, a list of all the mappings. More recently, the EWR tool itself provides its own causal networks. The version in HydroBOT is updated from these frequently and is tested, but to ensure the absolute most current causal networks, a user can use the `get_causal_ewr()` function to extract them from the EWR tool.
...
The theme relationships in the causal network `causal_ewr` provide the links and structure of the theme scaling, while the quantities to be scaled come out of the modules. For this demonstration, we provide a set of paths to point to the input data, in this case the outputs from the EWR tool, created by [a controller notebook](/controller/controller_overview.qmd).
