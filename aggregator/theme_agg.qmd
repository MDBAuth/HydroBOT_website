...
simpleColHistory <- multi_aggregate(
  dat = sumdat,
  causal_edges = make_edges(causal_ewr, aggseq),
  groupers = c("scenario", "gauge"),
  aggCols = "ewr_achieved",
...
annualColHistory <- multi_aggregate(
  dat = anndat,
...
aggvals <- aggvals %>%
  dplyr::filter(gauge == example_gauge) %>%
  dplyr::select(-gauge)
