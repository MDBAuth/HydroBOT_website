...
Here, we explore the spatial aggregator, `spatial_aggregate`, in isolation to discuss its specific characteristics, though in practice it is typically used within `multi_aggregate` and `read_and_agg` which wrap `spatial_aggregate`, `temporal_aggregate`, and `theme_aggregate` to allow interleaved aggregation steps in a standardised format. See the [page for multi_aggregate](theme_space_agg.qmd) and [read_and_agg](read_and_agg.qmd) for more details.
