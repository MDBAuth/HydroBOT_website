...
#                            'basin.shp'))
# 
# sdl_units <- sf::read_sf(file.path(geo_data_dir, 'resource_plan.shp')) %>%
#   sf::st_make_valid()
# 
# cewo_valleys <-sf::read_sf(file.path(geo_data_dir, 'cewo_valleys.shp')) %>%
#   sf::st_make_valid()
```
...
allagg$ewr_code %>% 
  ...
  ggplot() +
  geom_sf(data = basin) +
  geom_sf(data = cewo_valleys, aes(fill = ValleyName), alpha = 0.25, show.legend = F) + 
...
Still env_obj, now in sdl_units polys
...
Crossing sdl_units specific goals into catchments (using wm). Contrived, but good test.
