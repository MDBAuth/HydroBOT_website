...
format:
  html:
    df-print: paged
    # code-link: true # Doesn't do anything without either cran or pkgdown site
editor: visual
params:
  REBUILD_DATA: FALSE
---
...
Now we do the aggregation. Note that we have been very [aggressive in handling spatial processing](aggregator/aggregator_overview.qmd) and so while spatial processing is slow, we minimize it as much as possible internally.
...
Most often, we'll want to save the list of outcomes at each step- it allows us to see how we got the final outcome, and it's likely we're interested in the outcomes at more than one step anyway. As in the [theme notebook](aggregator/theme_agg.qmd), we do this with `saveintermediate = TRUE`. We also set `namehistory = FALSE` to put the aggregation tracking in columns instead of names for ease of handling the output.
...
That output is the same as `tsagg`, but now it's been read-in and processed in parallel over scenarios. As in the [theme situation](aggregator/theme_agg.qmd), this toy example is slower, but should yield large speedups for larger jobs.
