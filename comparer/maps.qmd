...
First, we read in the aggregated data. There is example data provided by HydroBOT (`agged_data` and `agged_data_colsequence`), but to continue with the demonstration, we will use the aggregations created here in the [interleaved aggregation notebook](../aggregator/using_multi_aggregate.qmd).
