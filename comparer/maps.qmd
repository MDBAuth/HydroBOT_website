...
Here, we use the special arguments `refvals` and `refcols` to make a color palette from a standard {paletter} option ("ggsci::nrc_npg") while setting a specific level to a specified value. We will use the [codes defining the scenarios](comparer_overview.qmd) rather than the names to make plots readable.
