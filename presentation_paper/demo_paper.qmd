...
```
*What aggregation do we want to use here? The dots here are the mean passing of each env_obj. Georiga had the mean EWRs pooled into env group. Those are different questions- one is just throwing all the EWRs in a bucket and asking about the proportion that passes, this is saying it doesn't matter how many EWRs go into an env_obj, what we're interested in is whether that env_obj passes. I think that's probably more relevant to management, but if we want to do it Georgia's way, we can use the data from agged_data_ewr2sdl I think.* 
