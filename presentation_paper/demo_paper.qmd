...
# qualitative axes
qual_heatmap <- agged_data_ewr2sdl$Target |>
...
quant_heatmap <- agged_data_ewr2sdl$Target |>
  dplyr::filter(adapt_code %in% c(1,2,3) &
                  scenario != 'MAX') |> # max isn't really relevant here
  dplyr::filter(!is.na(Target)) |> 
  # clean names
...
point_heatmap <- agged_data_ewr2sdl$Target |>
  dplyr::filter(adapt_code %in% c(1,2,3, 4) &
                  scenario != 'MAX') |> # max isn't really relevant here
  dplyr::filter(!is.na(Target)) |> 
  # clean names
...
contour_heatmap <- agged_data_ewr2sdl$Target |>
  dplyr::filter(adapt_code %in% c(1,2,3, 4) &
                  scenario != 'MAX') |> # max isn't really relevant here
  dplyr::filter(!is.na(Target)) |> 
  # clean names
...
NETWORK
  Full?
  Filtered to some focal group

### Additional?

Website? Does that go anywhere? Is it a result, or is that a method?
