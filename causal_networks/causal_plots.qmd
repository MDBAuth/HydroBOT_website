...
# SUPER TEMPORARY
# devtools::install_local(path = 'C:/Users/galen/Documents/WERP_toolkit/werptoolkitr')
devtools::load_all(path = 'C:/Users/galen/Documents/WERP_toolkit/werptoolkitr')
```
...
In standard R package structure, the data is in `.rda` format and located in `data/` and with standard names when read in.

The list of causal relationships is available by default with the package load as `causal_ewr`, so we don't need to read anything in here.
