...
(e.g. `returnType = list('summary', 'all')` in R. These have to be lists, not `c()` to work right when sent to Python- to make a `list` in python, need to have unnamed lists in R. The easiest to work with in HydroBOT is `'yearly'`, as that allows assessment of the outcomes, but we will return `'summary'` here as well.
