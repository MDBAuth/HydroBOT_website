...
The above is all user parameters. All the formatting, running, and saving is then handled with the wrapper function `prep_run_save_ewrs`. See [stepthrough](controller_ewr_stepthrough_R.qmd) for an expanded version used to run test data and expand each step to make testing/changes more transparent.
