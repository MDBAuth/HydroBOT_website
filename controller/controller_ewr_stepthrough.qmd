...
# have to triple ::: because this isn't exported
  hydro_paths <- as.list(HydroBOT:::find_scenario_paths(hydro_dir))
  
  # There's a bit more error catching etc in `prep_run_save_ewrs`, but ignoring that here.

  # extract scenarios if not given
  scenarios <- HydroBOT:::scenario_names_from_hydro(hydro_dir)
  
  # set up the output directory
  output_path <- HydroBOT:::make_output_dir(project_dir, 
                                                scenarios = scenarios, 
                                                module_name = 'EWR')
```
...
This is not actually run here for speed- the same thing is done in a notebook for the [the full workflow](../workflows/workflow_save_steps.qmd).
