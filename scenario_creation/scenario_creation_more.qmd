...
```{r}
pcg <- import("py_ewr.observed_handling")
catgauges <- pcg$categorise_gauges(ewr_demo_gauges) |>
  setNames(c('flow', 'level', 'stage'))

gauges_to_pull <- catgauges$flow
```
...
```{r}
demo_levs <- vicwater::get_ts_traces(state = 'NSW', 
...
```{r}
demo_geo = bom_basin_gauges %>% dplyr::filter(gauge %in% unique(demo_levs$site))
```
...
scenenames <- paste0(rep(scenenames,
                         times = length(unique(adaptadds))), 
                     stringr::str_sort(rep(sceneaddsnames,
...
if (params$REBUILD_DATA) {
  for (i in 1:length(scenenames)) {
    demo_levs %>% 
      ...
      dplyr::mutate(value = ifelse(lubridate::month(time) > sceneadapttimes[i],
