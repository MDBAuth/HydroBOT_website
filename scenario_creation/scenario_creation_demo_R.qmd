...
A larger and more complex set of scenarios is created in [the flow scaling demonstration](flow_scaling.qmd), without as much visualisation.
...
This notebook was originally built using only python, and there is still [a python-only version](scenario_creation_demo_py.qmd), though it is maintained less frequently. Using Python makes a lot of sense because the underlying data here uses python packages. I've moved the active version of this notebook to R, however, when the toolkit became an R package and the [flow scaling demonstration](flow_scaling.qmd) ended up using R gauge pullers. There is still some remaining python in here (pulling gauges and some minor EWR functions). This notebook provides an example of how to mix R and python code chunks, which we do fairly frequently.
...
The way that works everywhere but vscode
```{{r}}
...
The new version has to use the `reticulate::py_to_r`, because when we're not in the repl, we have an R object to work with for every line of python. 
```{r}
demo_levs <- demo_levs |> 
  dplyr::mutate(VALUE = as.numeric(VALUE))
demo_levs <- dplyr::mutate(demo_levs, Date = py_to_r(DATETIME[[1]])) |>
