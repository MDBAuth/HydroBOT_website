...
scale_gauges <- function(gaugedata, gaugename, qc_limit = 150) {
  ...
  gaugedata <- gaugedata %>%
    ...
    ungroup() %>%
    # join to correct sdl unit for the scalings If there are NA in the quantile
    # col (as there will be if there are NA values), this gives them all an NA
    # in the scenario column too. So I've given them negative quantiles, which we'll turn back to NA.
...
```{r}
plot_hydrographs(scaled_hydro[['418002']] %>% 
...
And now we can move on to read in to the toolkit.
