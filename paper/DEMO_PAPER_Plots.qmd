...
SWSDLName_to_plot <- c("Macquarie–Castlereagh", "Lachlan", "Namoi")
...
geom_sf(data = st_intersection(bom_basin_gauges, catch_demo), color = 'black')) +
...
saveRDS(system_plot, file = file.path(comp_results, "system_plot.Rdata"))
system_plot
```
...
# SF1 = > 200 ML/d, Anytime (ideally Oct–Apr), 10 days, Annually (10 years in 10) (100% of years)
# SF 2 = 200–1,000 ML/d, Sep–Apr (Sep–Dec for Murray cod spawning), 14 days, 5–10 years in 10 (75% of years)
...
saveRDS(Fig_5_PanelB, file = file.path(agg_results, "Fig_5_PanelB.Rdata"))
Fig_5_PanelB 
```
...
```{r}
Fig_5 <- Fig_5_PanelA + Fig_5_PanelB #+ plot_layout(guides = "collect") & theme(legend.position = 'bottom')
...
#NEED SOME WAY of knowing whether env objectives are not relevant or are not met when they don’t exist.
...
Fig_7 <- Fig_7a + Fig_7b + plot_layout(widths = c(3,1), guides = "collect") + plot_annotation(tag_levels = 'a') &
...
Fig_4 <- make_causal_plot(filter(nodes, NodeType != 'target_5_year_2024'),
                 edges, render = FALSE) %>% 
...
Fig_10 <- ggplot(data = env_theme_catchment_to_plot, aes(x = Climate, y = AdapOpt, fill = proportion_ewr_achieved)) + 
...
proj_meta <- list(Plannning_units = knitr::combine_words(SWSDLName_to_plot),
                  n_Plannning_units = words(length(SWSDLName_to_plot)),
                  gauges_to_plot = knitr::combine_words(gauges_to_plot),
                  n_gauges_to_plot = words(length(gauges_to_plot)),
